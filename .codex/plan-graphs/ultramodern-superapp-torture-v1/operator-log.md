...
| k6-load | `019de2d2-b20b-7083-9d86-74256aaf768e` (`Hilbert`) | worker | in_progress | Owns `ust-load-02`: deterministic k6 scenario definitions under `scripts/superapp-k6` plus k6-load plan status only. |
| chaos-failure | `019de2d2-b2b1-7212-b347-ac1124c5d820` (`Euler`) | worker | in_progress | Owns `ust-chaos-02`: deterministic failure toggles in SuperApp portfolio BFF/API/shared chaos tests plus chaos plan status only. |
| browser-runtime | `019de2d2-b338-71e3-9f81-7fe563b94152` (`Bernoulli`) | worker | in_progress | Owns `ust-browser-02`: production browser smoke/focused flows with console/pageerror/requestfailed/trace/screenshot/video artifacts plus browser plan status only. |
| effect-tanstack-contracts | primary | local | held | `ust-contract-02` is intentionally held until `ust-chaos-02` stabilizes failure toggles needed for timeout/retry classification; do not run BFF-affecting contract work in parallel. |
