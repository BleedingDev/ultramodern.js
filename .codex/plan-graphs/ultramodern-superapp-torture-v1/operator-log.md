...
| k6-load | `019de2fd-1153-7b70-848c-06d7b99131b1` (`Harvey`) | worker | in_progress | Owns `ust-load-05`: release/nightly thresholds and no-default-cost certification wiring under `scripts/superapp-k6` / narrow certification files plus k6-load plan status only. |
| chaos-failure | `019de2fd-1200-7230-9f64-bbcf394111e4` (`Mencius`) | worker | in_progress | Owns `ust-chaos-05`: deterministic chaos matrix artifact helper/tests plus chaos plan status only. |
| effect-tanstack-contracts | `019de2fd-1318-7011-a01d-990783729b10` (`Laplace`) | worker | in_progress | Owns `ust-contract-03`: Effect interruption/finalizer/schema defect/context propagation tests plus contract plan status only. |
| browser-runtime | `019de2fd-13f4-7c20-96b8-4652b824e4d9` (`Boole`) | worker | in_progress | Owns `ust-browser-05`: browser smoke subset under moderate load plus browser plan status only. |
