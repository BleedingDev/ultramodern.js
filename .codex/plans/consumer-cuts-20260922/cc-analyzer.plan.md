...
Keep resolution in-process and scoped to the import being analyzed. `strictEffectRuntimeTopologyViolation` deliberately removed per-module native compiler processes because Oxlint workers hit ENOMEM. Do not reintroduce that architecture or add a complete TypeScript program for this fix. `code-tools` currently ships Babel analysis, not a TypeScript runtime resolver; prove any reused resolver is actually available in the packed package, and hand a necessary dependency change to integration. Public export resolution does not expand the owner-local runtime implementation traversal in `createEffectApiImportResolver`.
