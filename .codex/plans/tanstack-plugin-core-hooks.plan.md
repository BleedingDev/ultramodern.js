...
`tpcore-04` is complete. Core-only tests now prove a fake non-TanStack plugin-owned route generation path, no generated TanStack output for that path, generic SSR snapshot helpers, and generic router state sources without direct `@tanstack/react-router` imports.
