...
Packed full-workspace acceptance exposed upstream Module Federation's native ESM CLI config callback calling an unbound `require`. The canonical modern-js-v3 patch now provides module-local createRequire in both ESM builds; CJS and DTS behavior are unchanged. Direct native ESM setup/config callbacks pass without global require and preserve remote-only lazy compilation. Inventory hash and generated patch assets were updated. Root regenerated the lock/install and rebuilt/repacked the generator. Final source-isolated packed workspace acceptance passed 3/3, including installed validation, routes-generate natural exit/artifacts and shell build (framework sources stripped and NODE_PATH cleared; /tmp/modernjs-fork-cuts-packed-workspace-qualified.log). Tractor full source-mode acceptance also passed at 5438498b2d0354d4a4396d802ef0b166f0a08cff with the exact 46-package cohort, Node/workerd SSR and visible shopping behavior.
