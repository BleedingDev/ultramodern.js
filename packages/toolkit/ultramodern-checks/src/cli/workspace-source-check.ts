...
const relativePath = (root: string, filePath: string) =>
...
const isLocaleJson = (root: string, filePath: string) =>
...
const runRuntimeAndLocaleResourceChecks = (
  ...
  for (const filePath of files.filter(filePath =>
    ...
    checkPluralResources(
      root,
      filePath,
      JSON.parse(readText(filePath)) as JsonValue,
    );
  }
};
...
export const runWorkspaceSourceCheck = ({
  ...
  const oxlintExitCode = runOxlintRules({
    cwd,
    targets: sourceRoots,
    rules: {
