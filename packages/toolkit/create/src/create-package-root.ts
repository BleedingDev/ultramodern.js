...
export function resolveCreatePackageRoot(fromDir: string): string {
  ...
  throw new Error(
    `Unable to resolve the @modern-js/create package root (a directory containing both package.json and template-workspace/) within ${MAX_WALK_UP_LEVELS} levels above ${fromDir}`,
  );
}
