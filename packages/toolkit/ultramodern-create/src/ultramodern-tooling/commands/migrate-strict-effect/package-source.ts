...
export function createMigrationPackageSource(
  ...
  if (isCreatePackageSourceCheckout() && explicitInstallRequested) {
    throw new Error(
      'A local @modern-js/ultramodern-create source checkout cannot migrate an explicit install package source. Use --workspace locally or run the packed published package with its authenticated release cohort projection.',
    );
  }
  const strategy =
