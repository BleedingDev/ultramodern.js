...
UltraModern.js V3 是构建在 Modern.js V3 之上的兼容增强发行版，目标是服务于大型、可独立部署的 Super App / ERP 场景。

本文是已发布 V3 线的完整公开差异文档。
...
| 应用级 MF SSR 合约开关 | 无专门 app-level MF SSR 稳定性契约开关 | 新增 `server.ssr.moduleFederationAppSSR` 分阶段稳定契约 | 已实现（分阶段稳定契约） | 注入 `process.env.MODERN_MF_APP_SSR` |
| 应用级 MF SSR 运行时桥接 | 非稳定默认桥接路径 | V3 仅提供合约/环境变量，完整桥接仍在分阶段推进 | 设计上部分支持 | 需受控灰度 |
...
| 合约测试覆盖 | 基线中无上述差异项配套测试 | 增加 RsDoctor、BFF requestId、Telemetry、SSR 分阶段契约环境变量的针对性测试 | 已实现 | 降低 super-app 场景回归风险 |
| 公开文档面 | 无 UltraModern V3 差异总览页 | 新增 V3 专用差异页并补齐关联配置文档 | 已实现 | 本页为 V3 权威基准 |
...
### 5) 应用级 MF SSR 分阶段契约（`server.ssr.moduleFederationAppSSR`）
...
:::warning
在 UltraModern.js V3 中，该开关已经提供稳定的配置/环境变量契约。完整的应用级 Module Federation SSR 运行时桥接仍处于分阶段推进中。
:::
...
UltraModern.js V3 在基线 Modern.js V3 之外补齐了可机审的发布 gate：
...
UltraModern 默认 create 模板会开启更严格的平台契约。以下环境变量会在生成的 `modern.config.ts` 中读取，可按应用或按环境关闭/覆盖：
...
- `MODERN_TELEMETRY_OTLP_ENDPOINT`（默认：`http://127.0.0.1:4318/v1/metrics`）：OTLP 导出端点。接入其他 OTLP collector 地址时覆盖。
