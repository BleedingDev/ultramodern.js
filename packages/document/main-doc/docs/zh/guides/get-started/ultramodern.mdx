...
| Starter 基线 Gate 工作流 | 生成脚手架默认没有更强默认值校验工作流 | 在 stronger-default starter 中新增 `baseline:check`、可关闭 env 开关与 `.github/workflows/uniform-baseline-gates.yml` | 已实现 | 让生成应用从第一笔提交起就可审计 |
...
| 公开文档面 | 无 UltraModern V3 差异总览页 | 新增 V3 专用差异页并补齐关联配置文档 | 已实现 | 本页作为这套 stronger-default 档案的公开参考页 |
...
这套 stronger-default Modern.js 档案在基线 Modern.js V3 之外补齐了可机审的发布 gate：
...
- 生成 starter 自带工作流：
  ...
  - 默认执行 `pnpm run baseline:check` 与 `pnpm run build`

## 不属于 UltraModern.js V3 的能力

以下能力刻意不纳入这套 stronger-default V3 档案：
...
默认的 stronger-default Modern.js create 模板会开启更严格的平台契约。以下环境变量会在生成的 `modern.config.ts` 中读取，可按应用或按环境关闭/覆盖：
