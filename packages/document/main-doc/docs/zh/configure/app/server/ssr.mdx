...
- `disablePrerender`: `boolean = false`, 为了兼容旧数据请求方式 - `useLoader`, 默认情况下 Modern.js 会对组件进行一次预渲染即有两次渲染。
  开发者在保证项目中没有使用 useLoader Api 情况下, 可通过配置 `disablePrerender=true`来减少一次渲染。
- `moduleFederationAppSSRAlpha`：`boolean = false`，开启应用级 Module Federation SSR 的 alpha 合约标记，并注入 `process.env.MODERN_MF_APP_SSR_ALPHA`。
...
:::warning
在 `release-v4` 中，`moduleFederationAppSSRAlpha` 当前只提供配置/环境变量合约能力。完整的应用级 Module Federation SSR 运行时桥接仍处于 alpha 推进阶段。
:::
