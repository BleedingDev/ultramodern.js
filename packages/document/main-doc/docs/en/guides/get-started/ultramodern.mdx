...
UltraModern.js V3 is a compatibility-first distribution built on top of Modern.js V3 for large, independently deployable super-app and ERP-style systems.

This page is the complete public difference reference for the published V3 line.
...
| App-level MF SSR contract flag | No dedicated app-level MF SSR stability contract switch | Adds `server.ssr.moduleFederationAppSSR` staged-stability contract | Implemented (staged-stability contract) | Sets `process.env.MODERN_MF_APP_SSR` |
| App-level MF SSR runtime bridge | Not a stable default bridge path | V3 exposes contract/env only, full bridge remains staged rollout | Partial by design | Use behind controlled rollout |
...
| Contract test coverage | No tests for these new contracts in V3 baseline | Adds focused tests for RsDoctor defaults, BFF requestId, telemetry, SSR staged-contract env wiring | Implemented | Reduces regression risk in super-app scenarios |
| Public docs surface | No UltraModern V3 comparison page | Adds dedicated V3 difference page and linked config docs | Implemented | This page is the canonical V3 reference |
...
### 5) App-level MF SSR staged contract (`server.ssr.moduleFederationAppSSR`)
...
:::warning
In UltraModern.js V3, this flag provides stable contract/env wiring. The full app-level Module Federation SSR runtime bridge is still a staged rollout item.
:::
...
- SSR staged-contract detection and env injection.
...
UltraModern.js V3 includes a machine-verifiable gate model beyond baseline Modern.js V3:
...
The following are intentionally out of scope for this published V3 line:
...
The default UltraModern create-template enables stricter platform contracts. These env switches are read by generated `modern.config.ts` and can be disabled/overridden per app or per environment:
