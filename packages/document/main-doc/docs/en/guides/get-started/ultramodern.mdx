...
UltraModern.js V3 documents a stronger-default Modern.js reference profile for large, independently deployable super-app and ERP-style systems.
...
| Starter baseline gate workflow | Generated starter has no stronger-default verification workflow | Adds `baseline:check`, opt-out env switches, and `.github/workflows/uniform-baseline-gates.yml` to the stronger-default starter | Implemented | Keeps generated apps auditable from the first commit |
...
| Public docs surface | No UltraModern V3 comparison page | Adds dedicated V3 difference page and linked config docs | Implemented | This page is the reference page for this stronger-default profile |
...
This stronger-default Modern.js profile includes a machine-verifiable gate model beyond baseline Modern.js V3:
...
- Generated starter workflow:
  - `.github/workflows/uniform-baseline-gates.yml`
  - runs `pnpm run baseline:check` and `pnpm run build`

## Not Part of UltraModern.js V3

The following are intentionally out of scope for this stronger-default V3 profile:
...
The default stronger-default Modern.js create template enables stricter platform contracts. These env switches are read by generated `modern.config.ts` and can be disabled/overridden per app or per environment:
