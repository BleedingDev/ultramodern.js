...
- Baseline: the current merged Modern.js `release-v3.x` baseline tracked by this fork; current fork packages are `3.2.0`, so this page should not be read as a frozen patch-version comparison.
