...
- `disablePrerender`: `boolean = false`, To ensure compatibility with the old data request method (`useLoader`), by default, Modern.js performs pre-rendering of components.
...
- `moduleFederationAppSSRAlpha`: `boolean = false`, enables the alpha contract flag for app-level Module Federation SSR and injects `process.env.MODERN_MF_APP_SSR_ALPHA`.
...
:::warning
In `release-v4`, `moduleFederationAppSSRAlpha` currently enables contract/env wiring only. Full app-level Module Federation SSR runtime bridging is still in alpha rollout.
:::
