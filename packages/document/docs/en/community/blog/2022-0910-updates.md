...
执行 dev 命令时，如果希望使用 Typescript 编译，需要安装 `Node-native TypeScript`，`tsconfig-paths` 到 `devDependencies`，否则默认仍然使用 Babel 编译。在 Modern.js 2.0 中，我们将使用 Node-native TypeScript 作为默认的编译器。
