...
- **Repack Order:** On `'repack'` the dev server awaits every handler before it purges the previous server bundle from the require cache. New requests wait until all handlers settle; requests already running keep the modules they loaded.
- **Example:**
