...
Sometimes in BFF functions, it's necessary to obtain the request context to handle more logic. In such cases, you can use `useBackendContext` to get it:
...
:::info
For more details, refer to [useBackendContext](/apis/app/runtime/bff/use-backend-context).
:::

### Getting Cookies

When getting cookies in BFF functions, you need to get the request context through `useBackendContext`, then use `c.req.header('cookie')` to get the Cookie string and parse it manually:
