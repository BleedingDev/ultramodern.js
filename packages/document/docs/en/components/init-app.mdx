...
When `--bff` or `--bff-runtime` is enabled, `api/lambda/hello.ts` is generated and `modern.config.ts` enables `@modern-js/plugin-bff`.
When `--bff-runtime effect` is enabled, `api/effect/index.ts` is generated and `bff.runtimeFramework` is set to `effect`.
