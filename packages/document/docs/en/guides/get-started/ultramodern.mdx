...
UltraModern.js serves here as the reference name for a stronger-default Modern.js profile. The goal is to harden the baseline for super-app and ERP-scale systems without forking Modern.js into a separate framework.
...
| Starter baseline enforcement | No generated baseline gate workflow | Generated starters include `baseline:check`, opt-out env switches, and `.github/workflows/uniform-baseline-gates.yml` |
...
4. The stronger-default profile now ships with explicit release and certification gates. The generated starter baseline also ships `.github/workflows/uniform-baseline-gates.yml`, so `pnpm run baseline:check` and `pnpm run build` stay part of the adoption contract from day one.

## Baseline Switches (Opt-out)

The default stronger-default Modern.js starter baseline enables strict platform contracts. Use these env switches to opt out per app or per environment:
