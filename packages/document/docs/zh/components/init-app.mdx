...
```bash
pnpm dlx @bleedingdev/modern-js-create myapp --router tanstack
```
...
```bash
pnpm dlx @bleedingdev/modern-js-create myapp --bff
```

初始化带 Effect HttpApi 运行时的 BFF 模板：

```bash
pnpm dlx @bleedingdev/modern-js-create myapp --bff-runtime effect
```
...
```bash
pnpm dlx @bleedingdev/modern-js-create myapp --router tanstack --bff-runtime effect --workspace
```
