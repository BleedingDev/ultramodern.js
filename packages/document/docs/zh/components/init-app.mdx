...
当启用 `--bff` 或 `--bff-runtime` 时，会生成 `api/lambda/hello.ts`，并在 `modern.config.ts` 中启用 `@modern-js/plugin-bff`。
当启用 `--bff-runtime effect` 时，会额外生成 `api/effect/index.ts`，并将 `bff.runtimeFramework` 设置为 `effect`。
