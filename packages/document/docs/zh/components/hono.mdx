...
在 BFF 函数中，有时需要获取请求上下文，来处理更多逻辑。此时，你可以通过 `useBackendContext` 来获取：
...
:::info
详细内容可以参考 [useBackendContext](/apis/app/runtime/bff/use-backend-context)。
:::

### 获取 Cookie

在 BFF 函数中获取 Cookie 时，需要通过 `useBackendContext` 获取请求上下文，然后使用 `c.req.header('cookie')` 获取 Cookie 字符串并手动解析：
