...
:::tip
如果项目通过 `--router tanstack` 创建，请在路由代码中使用 `@modern-js/plugin-tanstack/runtime`。如果直接使用内置运行时导出，请使用 `@modern-js/runtime/tanstack-router`。
:::
...
Modern.js 在独立 TanStack 插件和内置运行时导出中提供相同的辅助 API。

| 使用场景 | 运行时 API | 仅服务端 RSC 辅助函数 | 客户端 RSC 辅助函数 |
|---------|------------|------------------------|----------------------|
| 独立 TanStack 插件 | `@modern-js/plugin-tanstack/runtime` | `@modern-js/plugin-tanstack/runtime/rsc/server` | `@modern-js/plugin-tanstack/runtime/rsc/client` |
| 内置运行时导出 | `@modern-js/runtime/tanstack-router` | `@modern-js/runtime/tanstack-router/rsc/server` | `@modern-js/runtime/tanstack-router/rsc/client` |

通过 TanStack 插件创建的项目建议使用独立插件路径。下面的示例也使用该路径。
