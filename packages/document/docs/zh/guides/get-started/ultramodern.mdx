...
对于已经在使用 Modern.js v3 的团队，mvfh-06 的迁移路径是“兼容优先”：保持 Modern.js 的心智模型不变，逐步向 MV-first / TanStack-first / Effect-first 默认方向收敛，同时把兼容分支明确保留下来。
...
4. 将 mvfh-07 工作单独处理。这里仅说明兼容性与迁移步骤；发布 gate、认证流程和证据归属不属于 mvfh-06 范围。
