...
function extractCodestream(input: Uint8Array): Uint8Array | undefined {
  ...
  if (jxlcBox) {
    return input.slice(jxlcBox.contentOffset, jxlcBox.end);
  }
...
function extractPartialStreams(input: Uint8Array): Uint8Array[] {
  ...
  while (offset < input.length) {
    ...
    if (!jxlpBox) break;
    if (jxlpBox.end - jxlpBox.contentOffset < 4) break;
    partialStreams.push(input.slice(jxlpBox.contentOffset + 4, jxlpBox.end));
    offset = jxlpBox.end;
  }
...
export const JXL: IImage = {
  validate: (input: Uint8Array): boolean => {
    const signature = findBox(input, 'JXL ', 0);
    ...
    const brand = toUTF8String(
      input,
      ftypBox.contentOffset,
      ftypBox.contentOffset + 4,
    );
    return brand === 'jxl ';
  },
