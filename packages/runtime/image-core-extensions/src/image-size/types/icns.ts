...
export const ICNS: IImage = {
  ...
  calculate(input) {
    ...
    if (inputLength < SIZE_HEADER) {
      throw new TypeError('Invalid ICNS, truncated file header');
    }
    ...
    while (imageOffset < fileLength) {
      if (fileLength - imageOffset < SIZE_HEADER) {
      ...
      if (
        ...
        nextOffset > inputLength
      ) {
        throw new TypeError('Invalid ICNS, entry exceeds file bounds');
      }
