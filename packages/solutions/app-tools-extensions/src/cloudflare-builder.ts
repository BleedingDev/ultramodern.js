...
// data loaders; the Node server loads the same module as its loader bundle.
...
const createWorkerRouteDataModule = () =>
  [
    `import * as serverLoaderModule from './${SERVER_LOADER_COMBINED_FILE}';`,
 ...
 * route with a data loader. Bundle each entry's generated server loader module
 * behind the worker route data handler, mirroring the Node server loader
 * bundle, so the worker answers them instead of rendering the page.
 */
const getWorkerRouteDataEntries = (
  ...
  return Object.fromEntries(
    entrypoints.flatMap(({ entryName }) => {
      ...
      if (
        !fs.existsSync(path.join(entryDirectory, SERVER_LOADER_COMBINED_FILE))
      ) {
        return [];
      }
