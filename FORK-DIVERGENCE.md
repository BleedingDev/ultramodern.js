...
```json
{
  "schemaVersion": 1,
  "entries": [
    {"path":"packages/solutions/app-tools/src/plugins/deploy/platforms/node.ts","owner":"bleedingdev","reason":"Include server-plugin-declared runtime modules in the existing single ndepe dependency pass; configured adapter strings otherwise escape tracing and standalone deployments fall back to source-workspace packages. The fork BFF plugin owns adapter selection and declarations.","dispositions":["inline-patch"]},
...
| Upstream-owned path | Owner | Reason | Disposition |
| --- | --- | --- | --- |
| `packages/solutions/app-tools/src/plugins/deploy/platforms/node.ts` | bleedingdev | Include server-plugin-declared runtime modules in the existing single ndepe dependency pass; configured adapter strings otherwise escape tracing and standalone deployments fall back to source-workspace packages. The fork BFF plugin owns adapter selection and declarations. | `inline-patch` |
