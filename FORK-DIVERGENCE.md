...
```json
{
  ...
  "entries": [
    ...
    {"path":"packages/solutions/app-tools/src/plugins/deploy/utils/index.ts","owner":"bleedingdev","reason":"Allow the existing Node import resolver to use an application package as its referrer for declared deploy entries. The native deploy pass must select the same conditional export as the generated server instead of resolving from the framework tool installation.","dispositions":["inline-patch"]},
    ...
    {"path":"packages/server/server/package.json","owner":"bleedingdev","reason":"Retain reviewed native server tooling/dependency alignment and remove the reverse server-runtime-extensions dependency after higher solution composition owns fork plugins.","dispositions":["inline-patch","keep-[M]","upstream-PR"]},
...
| `packages/solutions/app-tools/src/plugins/deploy/utils/index.ts` | bleedingdev | Allow the existing Node import resolver to use an application package as its referrer for declared deploy entries. The native deploy pass must select the same conditional export as the generated server instead of resolving from the framework tool installation. | `inline-patch` |
...
| `packages/server/server/package.json` | bleedingdev | Retain reviewed native server tooling/dependency alignment and remove the reverse server-runtime-extensions dependency after higher solution composition owns fork plugins. | `inline-patch` + `keep-[M]` + `upstream-PR` |
