...
| `scripts/ultramodern-production-readiness` | Workflow runs published create proof (`.github/workflows/ultramodern-production-readiness.yml:91-95`); proof runs install, check, and build (`scripts/ultramodern-production-readiness/published-create-proof/main.mjs:139-158`). | Keep as release acceptance; add MV target scenarios once generator supports headless/separate-repo/external-zone flows. |
...
- No `pnpm check`, `pnpm build`, integration tests, or browser smoke were run; production readiness behavior is cited from source and workflow wiring only (`scripts/ultramodern-production-readiness/published-create-proof/main.mjs:139-158`, `.github/workflows/ultramodern-production-readiness.yml:91-95`).
