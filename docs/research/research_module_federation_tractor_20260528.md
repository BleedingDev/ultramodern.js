...
Our UltraModern scaffold is ahead on Cloudflare SSR, native Modern.js i18n, strict MF DTS, TypeScript native-preview, Effect BFF ownership, Zephyr bridge wiring, and generated contracts. It is behind the Tractor implementations on user-facing domain richness: the demo must model Explore/Decide/Checkout as visible full-stack micro-verticals, not a generic commerce page with a decorative boundary toggle.
...
This matters for our demo. A product detail page must visibly nest Checkout and Explore surfaces inside a Decide page. If `Add to cart` is outlined as Decide or the boundary border changes layout, the demo is teaching the wrong architecture.
...
| Gap | Why it matters |
| --- | --- |
| Generic vertical names | Tractor teaches domain ownership through Explore/Decide/Checkout; `remote-commerce` alone hides the split |
| Boundary visualizer semantics | Checkout controls must remain Checkout-owned even inside Decide pages |
| Boundary rendering | Thick borders that affect layout or overlap buttons make the visualizer misleading |
| Cart behavior | Tractor expects add, quantity update, remove, clear, minicart update, checkout, thanks |
| Route richness | Tractor includes home, category, stores, product detail, cart, checkout, thanks |
...
1. Promote the Tractor demo into the generated UltraModern starter as a real commerce reference mode.
...
3. Make boundary metadata a first-class generated contract.
   - Store boundary id, label, owner, color, and CSS custom properties in generated topology/contract JSON.
   - Render boundaries through data attributes and CSS variables, not hardcoded selectors only.

4. Fix the boundary visualizer as an overlay.
   - Use `outline`, inset `box-shadow`, or absolutely positioned pseudo-elements so toggling boundaries does not change layout metrics.
   - Use thin strokes and labels that sit outside controls without covering actions.
   - Validate with browser measurements: same scroll height and target bounding boxes before/after toggle.
...
7. Add a real checkout vertical.
   - Quantity increments, remove line item, clear cart, checkout form, store picker integration, thank-you state, and minicart update must be visible.
