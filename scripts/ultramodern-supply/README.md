...
`sidecars.json` pins each upstream npm tarball URL and SHA-512, identity, license, artifact set and the only allowed manifest transformations. `ipx.patch` records the Sharp object-form remap and CLI version update. Rsbuild image core has no runtime patch and depends on upstream image-size 2.0.3+, which bounds malformed parser loops. Its Sharp peer floor **remains >=0.35.4**.
