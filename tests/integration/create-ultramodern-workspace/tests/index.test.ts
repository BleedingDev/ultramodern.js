  ...
  test('scaffolds a shell-only UltraModern SuperApp workspace', async () => {
    ...
    for (const packagePath of appPackagePaths) {
      ...
      expect(packageJson.scripts['cloudflare:build']).toBe(
        'ULTRAMODERN_ZEPHYR=false MODERNJS_DEPLOY=cloudflare modern build && ULTRAMODERN_ZEPHYR=false MODERNJS_DEPLOY=cloudflare modern deploy --skip-build && node ../../scripts/generate-public-surface-assets.mts --app shell-super-app --target cloudflare',
      );
