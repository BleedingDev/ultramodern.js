...
5. Fork boundary — every change under `packages/**` is in exactly one of two buckets.
   ...
   - **Bucket B — changes to upstream-owned lines** (any file that exists at the audited merge-base recorded in `FORK-DIVERGENCE.md`) is allowed only as one of: (1) a PR to upstream `origin`, (2) use of an existing upstream extension point, or (3) a size-capped patch. Reject any Bucket-B hunk over ~20 lines per file per change; split it into Bucket A or send it upstream instead.
   - Every accepted Bucket-B divergence requires a `FORK-DIVERGENCE.md` entry — owner, reason, and a disposition drawn from that file's "Disposition vocabulary" table — authored in the same PR. No entry, no merge.
   - The enforcing gate is `node scripts/ultramodern-boundary-check/check-fork-import-boundary.js`. `--mode divergence` measures every upstream-owned file under `packages/` against the fixed upstream base `eded841256` (upstream's `Release v3.8.2 (#8810)` mainline commit — never the `v3.8.2` tag, which is a parallel commit and not an ancestor of `HEAD`), using per-file hunk and changed-line budgets from `scripts/ultramodern-boundary-check/divergence-allowlist.json` — never a PR merge-base for the divergence measurement. The allowlist is shrink-only relative to the PR merge-base: any raised budget, new file entry, or base re-anchor requires a same-PR `FORK-DIVERGENCE.md` change and the explicit `--record-growth` writer path. `allowlist.json` is a separate baseline used only by `--mode imports`; it may be refreshed with `--write-allowlist` when entries shrink. Do not confuse the two.
